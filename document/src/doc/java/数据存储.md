...
缓存K-V结构；计数器；
#### List
编码包括LINKEDLIST链表和ZIPLIST压缩列表。有序列表。

高性能分页；简单消息队列；
#### Hash
编码包括ziplist或者hashtable（数组+链表）
支持扩容和缩容；渐进式rehash，把原hashtable数据rehash到新的hashtable，过程中会记录rehash的index，进行可终端渐进式扩容或缩容。 
#### Set
编码是intset或者hashtable（value为空）
intset是一个整数集合，保存16位、32位、64位三种长度的整数。
无序集合。全局去重；集合的交集、并集、差集的操作。
#### Sorted Set
...
## 对比选型
MySQL：结构化数据；严格事务，数据一致性强；多表关联复杂查询；无官方分库分表；数据量瓶颈；
MongoDB：非结构化数据；数据分片扩展，海量数据；主从切换高可用；（自动过期、地理位置）无事务；单文档嵌套简单查询；
redis：高性能，10万qps；单线程，原子性；数据结构丰富；自动过期，当缓存；弱持久化；纯内存，成本高；数据量有限；
Hbase：